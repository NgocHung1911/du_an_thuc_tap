        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-blue-100 text-sm mt-1">
              {t('dashboard.admin_summary_subtitle')}
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            >
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500">{stats.inProgressTasks} {t('dashboard.status.IN_PROGRESS')} · {stats.todoTasks} {t('dashboard.status.TO_DO')}</p>
              </div>
            ...
            >
              <div className="space-y-1">
                ...
                <p className="text-xs text-emerald-600 font-semibold">{stats.taskCompletionRate}% {t('dashboard.status.COMPLETED')}</p>
              </div>
            ...
            >
              <div className="space-y-1">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t('dashboard.total_members')}</p>
                <p className="text-3xl font-extrabold text-purple-600">{stats.totalUsers}</p>
                <p className="text-xs text-slate-500">{stats.adminUsers} {t('dashboard.role.ADMIN')} · {stats.memberUsers} {t('dashboard.role.MEMBER')}</p>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    ...
                    <span>{t('dashboard.project status details')}</span>
                  </h2>
                  <button
                    onClick={() => navigate('/admin/my-projects')}
                    className="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1"
                  >
                    <span>{t('dashboard.view_list')}</span>
                    <ArrowRight size={14} />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-center space-y-1">
                    <p className="text-[11px] font-bold text-slate-500 uppercase">{t('dashboard.status_project.PLANNING')}</p>
                  ...
                  <div className="p-3.5 bg-sky-50 rounded-xl border border-sky-200 text-center space-y-1">
                    <p className="text-[11px] font-bold text-sky-700 uppercase">{t('dashboard.status_project.IN_PROGRESS')}</p>
                  ...
                  <div className="p-3.5 bg-emerald-50 rounded-xl border border-emerald-200 text-center space-y-1">
                    <p className="text-[11px] font-bold text-emerald-700 uppercase">{t('dashboard.status_project.COMPLETED')}</p>
                  ...
                  <div className="p-3.5 bg-amber-50 rounded-xl border border-amber-200 text-center space-y-1">
                    <p className="text-[11px] font-bold text-amber-700 uppercase">{t('dashboard.status_project.ON_HOLD')}</p>
